...
public:
     ...
     *
     * @param ss  A populated @c sockaddr_storage from the OS.
     *
     * @return On success, a @c Result<Address> holding the populated Address.
     ...
     * @throws Nothing — marked @c noexcept.
     */
    static Result<Address> from(const sockaddr_storage &ss) noexcept;
...
private:
    socklen_t size_{sizeof(sockaddr_in6)};  ///< Byte length of active address struct
    sockaddr_storage address_{};            ///< Storage large enough for both IPv4 and IPv6
    std::string ip_{};                      ///< Human-readable IP string
};
